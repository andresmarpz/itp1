...
function drawTreeInternal(x, y, width, height) {
  // brown
  fill(139, 69, 19);
...
function drawCanyonInternal(x, y, width, height) {
  fill(150, 75, 0);
...
function drawCollectableInternal(x, y, size) {
  fill(50, 200, 250);
  stroke(50);
  beginShape();
...
function drawFlagpole(flagpole) {
  stroke(130);
  strokeWeight(5);
  line(flagpole.x_pos, floor_y, flagpole.x_pos, floor_y - 250);
  noStroke();
  if (flagpole.isReached) {
    fill(255, 0, 0);
    rect(flagpole.x_pos, floor_y - 250, 50, 50);
  } else {
    fill(255, 255, 255);
    rect(flagpole.x_pos, floor_y - 50, 50, 50);
    stroke(130);
    strokeWeight(5);
    line(flagpole.x_pos, floor_y - 50, flagpole.x_pos, floor_y);
  }
}

function drawCollectable(collectable) {
  drawCollectableInternal(
    collectable.x_pos,
    collectable.y_pos,
    collectable.size
  );
...
function drawCanyon(canyon) {
  drawCanyonInternal(canyon.x_pos, canyon.y_pos, canyon.width, canyon.height);
}

function drawTree(tree) {
  drawTreeInternal(tree.x_pos, tree.y_pos, tree.width, tree.height);
}
